...
- 每条消息独立显示完整本地时间（年月日、时分秒），机器清单同时记录 ISO 8601、UTC 偏移
  和 Unix 时间，单独截取某条消息时仍能辨认其时间位置
...
每条消息的可见时间统一为 `YYYY年M月D日 HH:MM:SS`。HTML 的 `<time>` 元素和 JSON/CSV
清单另外记录带本机 UTC 偏移的 ISO 8601 时间，例如 `2026-08-03T16:48:27+08:00`，并保留
Unix 秒值。显示时间取自本机解析数据库时使用的系统时区；跨时区核验时应优先参考 ISO 8601
和 Unix 字段。
